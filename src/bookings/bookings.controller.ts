...
import { CheckoutPaymentIntent, Client, Environment, LogLevel, OrdersController, PaymentsController } from '@paypal/paypal-server-sdk';
...
export class BookingsController {
	...
	constructor(private readonly bookingsService: BookingsService, private readonly clsService: ClsService, private readonly showtimeService: ShowtimeService, private readonly usersService: UsersService, private readonly s3Service: S3Service) { 
	...
	async confirm(@Body() data: ConfirmBookingDto, @Param("id") id: string) {
		...
		const s3Object = await this.s3Service.generatePresignedUrl({fileName: booking.id + ".pdf", contentType: "application.pdf", domain: "tickets"});
