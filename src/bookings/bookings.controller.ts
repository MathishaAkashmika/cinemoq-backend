...
export class BookingsController {
	constructor(private readonly bookingsService: BookingsService, private readonly clsService: ClsService, private readonly showtimeService: ShowtimeService, private readonly usersService: UsersService) { }
	...
	async create(@Body() createBookingDto: CreateBookingDto) {
		...
		for (const seat of createBookingDto.seats) {
			const lockedRow = showtime.lockedSeats.find(s => s.col === seat.col && s.row === seat.row);
			if (lockedRow == null || lockedRow.userId.toString() !== user.id) {
				throw new HttpException(`Seat in row ${seat.row} and column ${seat.col} already locked`, HttpStatus.BAD_REQUEST);
			}

			await this.showtimeService.unlockSeat(showtime.id as string, user.id, seat.row, seat.col);
			await this.showtimeService.bookSeat(showtime.id as string, seat.row, seat.col);
		}

		const amount = showtime.price * createBookingDto.seats.length;
		const booking = await this.bookingsService.create(user.id, amount, createBookingDto);

		const merchantSecret = process.env.PAYHERE_MERCHANT_SECRET;
		const merchantId = process.env.PAYHERE_MERCHANT_ID;
		const orderId = booking.id;
		const hashedSecret = md5(merchantSecret);
		const amountFormated = amount.toLocaleString('en-us', { minimumFractionDigits: 2 }).replaceAll(',', '');
		const currency = 'LKR';
		const hash = md5(merchantId + orderId + amountFormated + currency + hashedSecret);
	...
	async confirm(@Body() data: ConfirmBookingDto, @Param("id") id: string) {
		...
		const unhashedSig = data.merchant_id + data.order_id + data.payhere_amount + data.payhere_currency + data.status_code + hashedSecret.toUpperCase();
		...
		if (sig === data.md5sig) {
			...
			if (code === 2) {
				...
				for (const seat of booking.seats) {
					await this.showtimeService.unbookSeat(booking.showtimeId.toString(), seat.row, seat.col);
				}
			}
		}
	}
